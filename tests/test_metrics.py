...
from hezar.metrics import Metric
...
class MetricsTestCase(TestCase):
    def test_accuracy(self):
        metric: Metric = build_metric("accuracy")
        predictions = [0, 2, 1, 3]
        references = [0, 1, 2, 3]
    ...
    def test_bleu(self):
        metric: Metric = build_metric("bleu")
    ...
    def test_cer(self):
        metric: Metric = build_metric("cer")
    ...
    def test_precision(self):
        metric: Metric = build_metric("precision")
        predictions = [0, 1, 2, 0, 1, 2]
        references = [0, 2, 1, 0, 0, 1]
        results = metric.compute(predictions, references)
        self.assertEqual(type(results), dict, INVALID_OUTPUT_TYPE)
    ...
    def test_recall(self):
        metric: Metric = build_metric("recall")
        predictions = [0, 1, 2, 0, 1, 2]
        references = [0, 2, 1, 0, 0, 1]
        results = metric.compute(predictions, references)
        self.assertEqual(type(results), dict, INVALID_OUTPUT_TYPE)
    ...
    def test_rouge(self):
        metric: Metric = build_metric("rouge")
    ...
    def test_seqeval(self):
        metric: Metric = build_metric("seqeval")
    ...
    def test_wer(self):
        metric: Metric = build_metric("wer")
        predictions = ["بهتره این یکی رو بیخیال شیم"]
        references = ["بهتره این یکی رو بیخیال بشیم"]
        results = metric.compute(predictions, references)
        self.assertEqual(type(results), dict, INVALID_OUTPUT_TYPE)
