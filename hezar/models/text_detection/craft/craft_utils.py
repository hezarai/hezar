...
def warp_coordinates(m_inv, pt):
    out = np.matmul(m_inv, (pt[0], pt[1], 1))
    return np.array([out[0] / out[2], out[1] / out[2]])


def get_boxes_core(textmap, linkmap, text_threshold, link_threshold, low_text):
    # prepare data
...
def get_poly_core(boxes, labels, mapper):
    ...
    for k, box in enumerate(boxes):
        ...
        # make final polygon
        poly = []
        poly.append(warp_coordinates(m_inv, (spp[0], spp[1])))
        for p in new_pp:
            poly.append(warp_coordinates(m_inv, (p[0], p[1])))
        poly.append(warp_coordinates(m_inv, (epp[0], epp[1])))
        poly.append(warp_coordinates(m_inv, (epp[2], epp[3])))
        for p in reversed(new_pp):
            poly.append(warp_coordinates(m_inv, (p[2], p[3])))
        poly.append(warp_coordinates(m_inv, (spp[2], spp[3])))
...
def get_detection_boxes(textmap, linkmap, text_threshold, link_threshold, low_text, poly=False):
    boxes, labels, mapper = get_boxes_core(textmap, linkmap, text_threshold, link_threshold, low_text)
